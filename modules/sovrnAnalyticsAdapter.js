...
const {
  EVENTS: { AUCTION_END, BID_REQUESTED, BID_ADJUSTMENT, BID_RESPONSE, BID_WON }
} = CONSTANTS;
...
let sovrnAnalyticsAdapter = Object.assign(
  adapter({ url: pbaUrl, analyticsType }),
  {
    track({ eventType, args }) {
      try {
        ...
        if (args.auctionId && currentAuctions[args.auctionId] === "complete") {
          throw new Error(
            "Event Recieved after Auction Close Auction Id " + args.auctionId
          );
        }
        if (args.auctionId && currentAuctions[args.auctionId] === undefined) {
          currentAuctions[args.auctionId] = new AuctionData(
            this.affiliateId,
            args.auctionId
          );
        }
...
sovrnAnalyticsAdapter.originEnableAnalytics =
...
sovrnAnalyticsAdapter.enableAnalytics = function(config) {
  ...
  if (config && config.options && config.options.affiliateId) {
    ...
    utils.logError(
      "Need affiliate Id to log auction results. Please contact a Sovrn representative if you do not know your affiliate Id."
    );
    return;
  }
...
class AuctionData {
   ...
   */
  findBid(event) {
    const bidder = this.auction.requests.find(
      r => r.bidderCode === event.bidderCode
    );
    if (!bidder) {
  ...
  adjustedBid(event) {
    ...
    if (bid) {
      bid.originalValues = Object.keys(event).reduce((o, k) => {
        if (
          JSON.stringify(bid[k]) !== JSON.stringify(event[k]) &&
          !this.dropBidFields.includes(k)
        ) {
          o[k] = bid[k];
          bid[k] = event[k];
        }
